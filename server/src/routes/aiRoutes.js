...
router.post('/match-resume-job', protect, async (req, res) => {
  try {
    ...
    // If the AI service returned 400 due to missing resume text, surface that
    // to the client so it can show a helpful prompt instead of silently hiding
    // the match section. This happens for new accounts whose resume text was
    // not yet extracted (e.g. AI service was unavailable during onboarding).
